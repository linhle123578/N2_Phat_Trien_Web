<?php 
    // --- LẤY HEADER (Chống tàng hình 100%) ---
    ob_start();
    include __DIR__ . '/../layouts/header.php';
    $raw_header = ob_get_clean();
    
    // Tìm thẻ <nav> hoặc <header>, nếu k có thì lấy toàn cục
                        ...
                        <div class="inner-card p-4 d-flex justify-content-between align-items-center flex-wrap gap-3">
                            <div>
                                <p class="fw-bold mb-1 fs-5" id="display-name">
                                    <?= htmlspecialchars($customer_info['fullname'] ?? 'Khách hàng') ?> 
                                    (+84) <?= htmlspecialchars($customer_info['phone'] ?? '000') ?>
                                </p>
                                <p class="text-gray mb-0" id="display-address">
                                    <?= htmlspecialchars($customer_info['address'] ?? 'Vui lòng cập nhật địa chỉ') ?>
                                </p>
                            </div>
    ...
    <div class="modal-overlay" id="address-modal">
        <div class="modal-content-custom">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="input-name" value="<?= htmlspecialchars($customer_info['fullname'] ?? '') ?>">
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-control form-select" id="input-address-type">
                    <option value="Nhà riêng" <?= (isset($customer_info['address_type']) && $customer_info['address_type'] == 'Nhà riêng') ? 'selected' : '' ?>>Nhà riêng</option>
                    <option value="Văn phòng" <?= (isset($customer_info['address_type']) && $customer_info['address_type'] == 'Văn phòng') ? 'selected' : '' ?>>Văn phòng</option>
                    <option value="Cơ quan" <?= (isset($customer_info['address_type']) && $customer_info['address_type'] == 'Cơ quan') ? 'selected' : '' ?>>Cơ quan</option>
                </select>
            </div>

            <div class="mb-4">
                <label class="form-label fw-bold">Địa chỉ chi tiết</label>
                <textarea class="form-control" id="input-address" rows="3"><?= htmlspecialchars($customer_info['address'] ?? '') ?></textarea>
            </div>
