...
<body>
    ...
    <section class="search-banner py-5">
        <div class="container text-center">
            ...
            <form method="GET"
      action="<?= BASE_URL ?>index.php" class="search-box mx-auto position-relative" style="max-width: 600px;">
            ...
            <aside class="col-lg-3">
                <div class="mb-4">
                    ...
                    <div class="list-group sidebar-list">
                        <a href="<?= BASE_URL ?>index.php?page=Products" class="list-group-item <?= empty($category_filter) ? 'active' : '' ?>">
                            Tất cả sản phẩm
                        </a>
                        
                        <?php while ($cat = mysqli_fetch_assoc($cat_result)): ?>
                            <a href="<?= BASE_URL ?>index.php?page=Products&category=<?= $cat['category_id'] ?>&search=<?= urlencode($search_filter) ?>&sort=<?= urlencode($sort_filter) ?>" 
                            ...
                            <div class="col-xl-4 col-md-4 col-6">
                                <div class="card h-100 product-card border-0 shadow-sm">
                                    <div class="position-relative">
                                        <a href="<?= BASE_URL ?>index.php?page=ProductDetail&id=<?= $product['product_id'] ?>" class="d-block">
                                ...
                                <a href="<?= BASE_URL ?>index.php?page=ProductDetail&id=<?= $product['product_id'] ?>" class="product-title-link">
                        ...
                        <nav aria-label="Page navigation">
                            <ul class="pagination custom-pagination-wrapper mb-0">
                                <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                                    <a class="page-link" href="<?= BASE_URL ?>index.php?page=Products&p=<?= $page - 1 ?><?= !empty($category_filter) ? '&category='.$category_filter : '' ?><?= !empty($search_filter) ? '&search='.urlencode($search_filter) : '' ?>&sort=<?= $sort_filter ?>"><i class="fas fa-chevron-left"></i></a>
                                </li>
                                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                    <li class="page-item <?= ($page == $i) ? 'active' : '' ?>">
                                        <a class="page-link" href="<?= BASE_URL ?>index.php?page=Products&p=<?= $i ?><?= !empty($category_filter) ? '&category='.$category_filter : '' ?><?= !empty($search_filter) ? '&search='.urlencode($search_filter) : '' ?>&sort=<?= $sort_filter ?>"><?= $i ?></a>
                                    </li>
                                <?php endfor; ?>
                                <li class="page-item <?= ($page >= $total_pages) ? 'disabled' : '' ?>">
                                    <a class="page-link" href="<?= BASE_URL ?>index.php?page=Products&p=<?= $page + 1 ?><?= !empty($category_filter) ? '&category='.$category_filter : '' ?><?= !empty($search_filter) ? '&search='.urlencode($search_filter) : '' ?>&sort=<?= $sort_filter ?>"><i class="fas fa-chevron-right"></i></a>
                                </li>
                            </ul>
                        </nav>
                    </div>
