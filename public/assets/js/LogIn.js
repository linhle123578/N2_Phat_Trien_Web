document.addEventListener("DOMContentLoaded", function () {
        ...
        formLogIn.addEventListener("submit", function (e) {
            ...
            fetch(fetchUrl, {
                ...
                .catch(err => {
                    console.error(err);
                    serverAlert.className = "alert alert-danger";
                    serverAlert.textContent = "Không thể kết nối với hệ thống Cloud. Chi tiết: " + err.message;
                    submitBtn.disabled = false;
                    submitBtn.textContent = "Đăng nhập";
                });
        });
    ...
    if (formReset) {
        formReset.addEventListener("submit", function (e) {
            ...
            if (otp.length !== 6) {
                document.getElementById("msg-otp").textContent = "Vui lòng nhập mã OTP 6 chữ số.";
                hasError = true;
            }
            if (newPass.length < 6) {
